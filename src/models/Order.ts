...
export class OrderModel {
   ...
   * Create a new order
   */
  async create(shopifyOrder: ShopifyOrder, customerId: number | null): Promise<number> {
    const result = await this.db.run(
      `INSERT INTO orders (shopify_order_id, customer_id, total_price, order_status, created_at) 
       VALUES (?, ?, ?, ?, ?)`,
      [
  ...
  async exists(shopifyOrderId: string): Promise<boolean> {
    const existingOrder = await this.db.get<{ order_id: number }>(
      'SELECT order_id FROM orders WHERE shopify_order_id = ?',
      [shopifyOrderId]
    );
   ...
   */
  async getAllSummaries(): Promise<OrderSummaryDTO[]> {
   ...
   */
  async getByShopifyId(shopifyOrderId: string): Promise<Order | undefined> {
    return this.db.get<Order>(
   ...
   */
  async markCompleted(orderId: number): Promise<void> {
    await this.db.run(
